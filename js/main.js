...
function initAccount() {
  ...
  const yrs = Array.from(new Set(
    Object.keys(pay).map(k=>k.match(/_(\d{4})$/)).filter(Boolean).map(m=>m[1])
  )).sort((a,b)=>b-a);
  ...
  const buildGroup = (cats, selector) => {
    const tbody = $(selector); if (!tbody) return;
...
function initUpdateForm() {
  ...
  quotaEl.id='updateQuotaInfo'; quotaEl.className='col-12 small text-muted mt-0';
  ...
  f.addEventListener('submit', async e=>{
    ...
    try {
      let lim=JSON.parse(localStorage.getItem(limitKey)||'null')||{date:todayStr(),count:0};
      if(lim.date!==todayStr()) lim={date:todayStr(),count:0};
      if(lim.count>=MAX){ fb.textContent=`You have reached the limit (${MAX} updates today).`; refreshQuota(); btn.disabled=false; return; }
      const cur=f.currentPassword.value.trim();
      const np=f.password.value.trim();
      const cp=f.confirmPassword.value.trim();
      if(np&&checkMissing(np)>0){ fb.textContent='Password is not strong enough.'; btn.disabled=false; return; }
      if(np&&np!==cp){ fb.textContent='Passwords do not match.'; btn.disabled=false; return; }
      const sb=await __ensureSB(); const p={};
      ...
      sessionStorage.removeItem('userData'); location.href='/membership/';
    } catch(err){ console.error(err); fb.textContent='Unexpected error. Please try again.'; btn.disabled=false; }
  });
}
...
function initMiscUI() {
  ...
  const doLogout=()=>{ sessionStorage.removeItem('userData'); location.href='/'; };
  ['btnLogoutFooter','btnLogoutNav'].forEach(id=>{
    const el=document.getElementById(id); if(el) el.addEventListener('click',doLogout);
  });
  const doPrint=()=>window.print();
  ['btnSaveScreenshot','btnSaveScreenshotNav'].forEach(id=>{
    const el=document.getElementById(id); if(el) el.addEventListener('click',doPrint);
  });
...
function initEventCarousel() {
  ...
  function loop(){ if(!paused){ track.scrollLeft+=speed; if(track.scrollLeft>=track.scrollWidth-track.clientWidth) track.scrollLeft=0; } requestAnimationFrame(loop); }
  loop(); ['mouseenter','focus'].forEach(evt=>track.addEventListener(evt,()=>paused=true)); ['mouseleave','blur'].forEach(evt=>track.addEventListener(evt,()=>paused=false));
...
document.addEventListener('DOMContentLoaded',()=>{
  initLogin();
  if(location.pathname.startsWith('/account/')) initAccount();
  initMiscUI();
  initAOSandDeepLink();
  initEventCarousel();
  document.querySelectorAll('.pp-toggle').forEach(btn=>btn.addEventListener('click',()=>{
